...
Por su parte, tendremos dos aplicaciones cliente, con dos interfaces de usuario diferentes. La aplicación móvil
 siguen el patrón de arquitectura Modelo-Vista-Controlador (MVC). 


Por su parte, en las aplicaciones cliente, la interfaz web del servidor de aplicaciones y la interfaz del cliente móvil, siguen el patrón de arquitectura Modelo-Vista-Controlador (MVC) y Modelo-Vista-VistaModelo (MVVM), respectivamente. En ellas, el modelo no se encuentra implementado en la propia aplicación, sino se accede, a través de la capa de acceso, a los servicios ofrecidos por la capa de servicios del modelo. De esta forma, ambas aplicaciones finales, invocan al modelo sin necesidad de tenerlo replicado.
...
En la figura 6.8 se muestra un esquema, en el que se define e implementa un \textit{DAO} genérico que proporciona las principales operaciones de acceso a datos, permitiendo que las definiciones de \textit{DAOs} que extiendan a este genérico tengan loas operaciones básicas sin necesidad de definirlas ni implementarlas . Este ejemplo, para el \textit{DAO} de la entidad \textit{User} se compone de:
...
Este esquema se repite con todas las entidades, exceptuando aquellas que necesitan de una relación con otra entidad para existir. Por ejemplo, la entidad \textit{RouteDay} necesita de \textit{Route} para existir, por lo que su identificador es dependiente de la otra entidad. Para este tipo de entidades, que no pueden seguir las definiciones del \textit{DAO} genérico explicado anteriormente por estar diseñado únicamente para entidades formadas por identificadores de un único atributo, es necesario presentar una solución diferente, donde cada \textit{DAO} define e implementa sus funcionalidades necesarias. 

Este diseño y el de todos los \textit{DAOs} se puede observar en el Apéndice .... (Añadir a apendice)
...
En este diagrama se definen dos clases: \textit{BaseClient} y \textit{ClientUser}. La primera es la encargada de crear un cliente que permita construir peticiones \textit{HTTP} que se usarán para invocar los métodos ofrecidos en la capa de servicios.

Por su parte, la clase \textit{ClientRoute}, modifica el comportamiento de la clase anterior indicando sobre qué recurso tiene que elaborar las peticiones \textit{HTTP}. En este ejemplo, se indica el recurso \textit{UserResource} de la capa de servicios. Análogamente, se crean las diferentes clases para acceder a los diferentes recursos creados en la capa de servicios del modelo.
...
En este ejemplo se especifica el controlador \textit{MyRoutesController} que define sobre una \textit{url} los métodos que se pueden realizar para la gestión de las rutas propias de un usuario.
\begin{itemize}
	...
	\begin{itemize}
		\item \textbf{get: } Recibe las peticiones \textit{GET} sobre la url especificada.
...
\begin{itemize}
	\item En el patrón Modelo-Vista-Controlador se separan los datos de una aplicación en los tres componentes anteriormente mencionados. Cuando la lógica de negocio realiza un cambio, este es necesario que sea actualizado en la vista.
...
Para el cliente móvil se definen un conjunto de clases que permitan ejecutar los métodos de la capa de servicios. \textit{UserServiceProvider} es una de ellas, en la que se crea y elabora la petición \textit{HTTP} necesaria para ejecutar los diferentes métodos ofrecidos por la capa servicios del modelo.
...
Este diseño de autenticación surge debido al requisito funcional para el cliente móvil, que exige mantener la sesión abierta, sin necesidad de loguearse de nuevo, cada vez que la aplicación es iniciada. Con este diseño, nuestro cliente se autentica en la aplicación y consigue un \textit{access token} y un \textit{refresh token}. Cuando el \textit{access token} caduca y deja de ser válido en el tiempo, el sistema devuelve una excepción específica, de tal manera, que el cliente móvil, sabrá que tendrá que actualizar dicho \textit{access token} realizando una petición al servidor indicando su \textit{refresh token}. 
