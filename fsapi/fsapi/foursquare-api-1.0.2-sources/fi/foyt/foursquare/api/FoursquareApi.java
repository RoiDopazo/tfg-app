...
public class FoursquareApi {
  ...
  public void setVersion(String version) {
    ...
    /**
   * Sets Foursquare API locale
   *
   * @param locale
   *          Foursquare API locale
   */
   ...
   * Returns URLs or identifiers from third parties that have been applied to this venue
   *
   * @see <a href="https://developer.foursquare.com/docs/venues/links.html" target="_blank">https://developer.foursquare.com/docs/venues/likes.html</a>
   *
   * @param id id of the venue
   * @return LinkGroup entity wrapped in Result object
   * @throws FoursquareApiException when something unexpected happens
   */
  public Result<LikeGroup> venuesLikes(String id) throws FoursquareApiException {
    try {
      ApiRequestResponse response = doApiRequest(Method.GET, "venues/" + id + "/likes", isAuthenticated());
      LikeGroup result = null;

      if (response.getMeta().getCode() == 200) {
        result = (LikeGroup) JSONFieldParser.parseEntity(LikeGroup.class, response.getResponse().getJSONObject("likes"), this.skipNonExistingFields);
      }

      return new Result<LikeGroup>(response.getMeta(), result);
    } catch (JSONException e) {
      throw new FoursquareApiException(e);
    }
  }
